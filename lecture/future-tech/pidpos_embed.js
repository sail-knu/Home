...
function demoVisible() {
  ...
  var block = document.getElementById('sim-pidpos');
...
var P = { mc: 1, mb: 1, L: 2, b: 0, kP: 100, kI: 0, kD: 40, kPx: 0, kDx: 0 };
var X0 = 1.2;
var S = {
  x: X0, v: 0, th: TH0, om: 0, F: 0, t: 0,
  iTh: 0, iLast: 0, iOn: false,
  running: false, fell: false, fellT: 0
};

var cv = document.getElementById('pidpos-cv');
if (!cv) return;
...
function updateCtrl(dt) {
  ...
  if (P.kI !== 0) {
    ...
    S.iTh = 0; S.iLast = 0; S.iOn = false;
  }
  F += P.kPx * S.x + P.kDx * S.v;
  S.F = F;
}

function reset() {
  S.x = X0; S.v = 0; S.th = TH0; S.om = 0;
...
function syncReadout() {
  var px = document.getElementById('pidpos-kPxV');
  var dx = document.getElementById('pidpos-kDxV');
  var th = document.getElementById('pidpos-th');
  var xv = document.getElementById('pidpos-x');
  var fv = document.getElementById('pidpos-F');
  if (px) px.textContent = P.kPx.toFixed(1);
  if (dx) dx.textContent = P.kDx.toFixed(1);
  if (th) th.textContent = fx(S.th * 180 / Math.PI, 1) + '°';
  if (xv) xv.textContent = fx(S.x, 2) + ' m';
  if (fv) fv.textContent = fx(S.F, 1) + ' N';
  var play = document.getElementById('pidpos-btnPlay');
  if (play) {
...
(function bind() {
  var slPx = document.getElementById('pidpos-kPx');
  var slDx = document.getElementById('pidpos-kDx');
  if (slPx) {
    slPx.value = String(P.kPx);
    slPx.oninput = function () { setGain('kPx', slPx.value, 0, 10); };
  }
  if (slDx) {
    slDx.value = String(P.kDx);
    slDx.oninput = function () { setGain('kDx', slDx.value, 0, 15); };
  }
  var play = document.getElementById('pidpos-btnPlay');
  var rst = document.getElementById('pidpos-btnReset');
